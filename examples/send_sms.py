...
def create_client(user: str = None, password: str = None, token: str = None, host: str = None, use_json=True) -> QTSMSClient:
    """Create QTSMSClient with appropriate authentication method."""
    if token:
        return QTSMSClient.with_token_auth(api_key=token, host=host)
    return QTSMSClient(user=user, password=password, host=host, use_json=use_json)
...
def print_success(response, verbose: bool = False):
    ...
    if hasattr(response, 'actions'):
        for action in response.actions:
            ...
            if action.post_id:
                print(f"  Post ID: {action.post_id}")
...
def print_error(error: Exception, error_code_class, verbose: bool = False):
    ...
    for code, msg in error_code_class.get_all_errors().items():
        if str(code).lower() in error_msg.lower():
            description = msg
...
async def send_sms_async(client, phone, text, sender, post_id):
...
Examples:
  ...
  %(prog)s --token my_token --phone +79991234567 --text "Hello World" --sender MyCompany
    ...
    parser.add_argument("--post-id", type=str, default=None, help="Optional post ID")
    parser.add_argument("--host", type=str, default=None, help="SMS gateway host (default: a2p-sms.beeline.ru)")
    parser.add_argument("--use-json", action="store_true", default=False, help="Use JSON format")
    ...
    try:
        client = create_client(user=args.user, password=args.password, token=args.token,
                               host=args.host, use_json=args.use_json)
        
        if args.verbose:
            print(f"Sending SMS to {args.phone}")
            print(f"via: {client.base_url} as {client.user}:{client.password}")
